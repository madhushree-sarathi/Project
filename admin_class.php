...
Class Action {
	...
	function get_parcel_heistory(){
		...
		if($parcel->num_rows <=0){
			...
			$data[] = array('status'=>'Request Raised','date_created'=>date("M d, Y h:i A",strtotime($parcel['date_created'])));
			$history = $this->db->query("SELECT * FROM parcel_tracks where parcel_id = {$parcel['id']}");
			$status_arr = array("Request Raised","Verified By Dean","CSPU","Approved By Dean","Approved By Principal","Successfully Approved");
